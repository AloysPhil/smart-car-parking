...
function allProducts() {
    ...
    $.ajax({
        ...
        url: 'events.php', // get the route value
        success: function(response) { //once the request successfully process to the server side it will return result here
            ...
            if (response.length) {
                ...
                $.each(response, function(key, value) {
                    ...
                    var link = "'./event.html?trip_no=" + value.id + "'";
                    
                    var isoFormatDateString = value.eventDate;
                    ...
                    var event_date = day + ", " + date + " " + month + " " + year;
                    
                    var isoFormatTimeString = value.eventTime;
                    
                    var timeParts = isoFormatTimeString.split(":");
                    ...
                    html += '<div class="image-thumb"><img src="../assets/images/events/posters/' + value.image + '" alt =""></div>';
                    html += '<div class="down-content mt-1">';
                    html += '<h3 class="mb-2">' + value.title + '</h3>';
                    html += '<h5 class="mb-1">' + event_date + " " + time + '</h5>';
                    html += '<h5 class="mb-2">' + value.venue + '</span></h5>';
                    html += '<h4 class="mb-1" style="color: #542e1a;">Ksh ' + value.price + '</h4>';
                    html += '</div>';
                    html += '</div>';
                    html += '</div>';
